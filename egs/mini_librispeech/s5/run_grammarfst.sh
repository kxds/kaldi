...
graph_dir=data/lang_test_grammar2
lm_file=
...
#   "<UNK>" data/local/lang_tmp_nosp data/lang_test

if [ $stage -le 3 ]; then

  [ -f data/lang_test_grammar1/G.fst ] && rm data/lang_test_grammar1/G.fst
  utils/prepare_lang.sh data/local/dict_nosp_grammar1 \
       "<SPOKEN_NOISE>" data/local/lang_tmp_nosp data/lang_test_grammar1
    
  rm -r  data/lang_test_grammar2{a,b} 2>/dev/null || true
  cp -r data/lang_test_grammar1 data/lang_test_grammar2a
  cp -r data/lang_test_grammar1 data/lang_test_grammar2b
  cp -r data/lang_test_grammar1 data/lang_test_grammar2

  python3 get_relabel.py data/lang_test_grammar1/words.txt "$nonterminal"
fi

if [ $stage -le 2 ]; then
  # Create a simple G.fst in data/lang_test_grammar1, which won't
  # actually use any grammar stuff, it will be a baseline to test against.

  lang=data/lang_test_grammar1
...
if [ $stage -le 3 ]; then
  # create the top-level graph in data/lang_test_grammar2a
...
#   cp -r $lang/phone $lang/graph/.
  arpa2fst --disambig-symbol=#0 --max-arpa-warnings=-1 --read-symbol-table=$lang/words.txt \
          $lm_file $lang/G.fst
  fstrelabel --relabel_opairs=relabel.txt $lang/G.fst $lang/b.fst
  mv $lang/b.fst $lang/G.fst
  utils/mkgraph.sh --self-loop-scale 1.0 $lang $tree_dir $graph_dir/grammar2a

fi

if [ $stage -le 4 ]; then
  # Create the graph for the nonterminal in data/lang_test_grammar2b
  # Again, we don't choose to put these symbols on the output side, but it would
  # be possible to do so.
  lang=data/lang_test_grammar2b
  for nonterm in $nonterminal;do
